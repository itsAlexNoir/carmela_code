...
"""Colour stuff: Define colour variables for nice plotting"""
...
def plotprob1d(func,axes,xlim=None,ylim=None,
    ...
    """Function for easy plotting 1D functions. All the parameters are fixed in advance.
    You just plug your function. See the docs for specifying more parameters.

    :param func: Actual function to be plotted
    :param axes: 
    :param xlim: 
    :param ylim: 
    :param xticks: 
...
def animate2frames(it,mat1,mat2,surf1,surf2,dvol1, dvol2, 
    filename1,filename2,pulse,time,efield):
    
    print('time: %0.4f fs'.format(time[it])) 
    # Initialize to zero
    mat1[:,:] = 0.0
    mat2[:,:] = 0.0

    mat1, ntime = data.load_h5series(mat1,filename1,it)
    mat2, ntime = data.load_h5series(mat2,filename2,it)
    
    mat1[:,:] *= dvol1
    mat2[:,:] *= dvol2
    
    surf1.set_array(np.ravel(np.log10(mat1)))
    surf2.set_array(np.ravel(np.log10(mat2)))
    
    pulse.set_data(time[it],efield[it,1])
    time_text.set_text('Time: %3.2f fs' %(time[it]))
    return pulse,time_text,mat1,mat2
  	
################################################################

def animate1frame(it,mat1,surf1,dvol1,filename1,pulse,time,efield):
    print('time: %0.4f fs'.format(time[it]))
    # Initialize to zero
    mat1[:,:] = 0.0
    ntime = data.load_h5series(mat1,filename1,it)
    mat1[:,:] *= dvol1
    surf1.set_array(np.ravel(np.log10(mat1)))
